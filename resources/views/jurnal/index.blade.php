...
<body class="dashboard-body">
            ...
            <ul class="dash-menu">
                ...
                <li class="dash-menu-category">
                    ...
                    <ul class="dash-sub-menu" id="akademikSub" style="display: flex;">
                        ...
                        <li>
                            <a href="{{ route('jurnal.index') }}" class="dash-sub-link" style="background-color: var(--dash-navy); color: #ffffff; font-weight: 700;">
        ...
        <main class="dash-main">
            ...
            <div class="jurnal-controls-row">
                ...
                <div class="jurnal-action-group">
                    ...
                    <a href="{{ route('jurnal.create') }}" class="btn-jurnal-tambah">
            ...
            <!-- ---------------------------------------------------------------
                 ...
                 --------------------------------------------------------------- -->
            <div class="jurnal-card-container">
    ...
    <script>
        ...
        function showToast(message, type = 'success') {
            ...
            toast.style.cssText = `background-color: ${bgColor}; border: 1px solid ${borderColor}; color: ${textColor}; padding: 12px 18px; border-radius: 12px; font-size: 0.9rem; font-weight: 600; margin-bottom: 12px; transition: all 0.4s ease;`;
        ...
        function reloadJurnalTable() {
            ...
            currentFilters.id_guru = document.getElementById('filterGuru').value;
            currentFilters.id_kelas = document.getElementById('filterKelas').value;
            currentFilters.id_mapel = document.getElementById('filterMapel').value;
            ...
            Object.keys(currentFilters).forEach(k => { if (currentFilters[k]) params.append(k, currentFilters[k]); });
            ...
            .then(resData => {
                ...
                // Update stats
                if (resData.stats) {
                    document.getElementById('statTotal').innerText = resData.stats.total_pertemuan;
                    document.getElementById('statTerlaksana').innerText = resData.stats.terlaksana;
                    document.getElementById('statBelum').innerText = resData.stats.belum_terlaksana;
                    document.getElementById('statGuru').innerText = resData.stats.total_guru_aktif;
                }

                if (!resData.data || resData.data.length === 0) {
                    tbody.innerHTML = `<tr><td colspan="8" style="text-align: center; padding: 40px; color: #847e73;">Belum ada data jurnal mengajar.</td></tr>`;
                } else {
                    tbody.innerHTML = resData.data.map(j => {
                        ...
                        return `
                            <tr id="row-jurnal-${j.id_jurnal}">
                                ...
                                <td>${guruCell}</td>
                                <td><div style="max-width: 200px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="${j.materi}">${j.materi}</div></td>
                                ...
                                <td>
                                    <div class="action-icons-cell">
                                        ...
                                        <button type="button" class="action-btn-icon delete" title="Hapus" onclick="deleteJurnalAjax(${j.id_jurnal})">
                                            <svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                                <polyline points="3 6 5 6 21 6"></polyline>
                                                <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path>
                                            </svg>
        ...
        /* ---- Search ---- */
        let searchTimer;
        document.getElementById('jurnalSearchInput').addEventListener('input', function() {
            clearTimeout(searchTimer);
            currentFilters.search = this.value;
            searchTimer = setTimeout(reloadJurnalTable, 400);
        });
        ...
        function openViewModal(id) {
            ...
            .then(data => {
                ...
                document.getElementById('view_status').innerHTML = data.status_kehadiran === 'Hadir'
                    ? '<span class="badge-status-terlaksana">Terlaksana</span>'
                    : '<span class="badge-status-belum">' + data.status_kehadiran + '</span>';
                ...
                document.getElementById('view_hadir').innerText = data.jumlah_hadir;
                document.getElementById('view_tidak_hadir').innerText = data.jumlah_tidak_hadir;
        ...
        function deleteJurnalAjax(id) {
            ...
            .then(res => res.json())
            .then(data => {
                showToast(data.success || 'Jurnal berhasil dihapus.');
                reloadJurnalTable();
            });
        }
