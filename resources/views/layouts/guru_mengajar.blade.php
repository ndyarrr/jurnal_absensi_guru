...
<body class="dashboard-body">
    ...
    <div class="dash-layout">
        ...
        <aside class="dash-sidebar">
            <div>
                ...
                <ul class="gm-nav-menu">
                    ...
                    <li>
                        <a href="{{ route('guru-mengajar.izin') }}" class="gm-nav-link {{ request()->routeIs('guru-mengajar.izin*') ? 'active' : '' }}">
        ...
        <main class="dash-main">
            ...
            @if(session('success'))
                <div class="gm-alert success" style="margin: 20px 24px 0 24px;">
            ...
            @if(session('bot_info'))
                @php
                    $isSuccess = session('bot_info_type') === 'success';
                @endphp
                <div class="gm-alert" style="margin: 12px 24px 0 24px; background: {{ $isSuccess ? '#f0fdf4' : '#fffbe6' }}; border: 1px solid {{ $isSuccess ? '#bbf7d0' : '#ffe58f' }}; color: {{ $isSuccess ? '#166534' : '#d48806' }}; padding: 12px 18px; border-radius: 10px; font-weight: 600; font-size: 0.875rem; display: flex; align-items: center; gap: 10px;">
                    <i class="fa-solid {{ $isSuccess ? 'fa-circle-check' : 'fa-triangle-exclamation' }}" style="font-size: 1.2rem; color: {{ $isSuccess ? '#22c55e' : '#eab308' }}; flex-shrink: 0;"></i>
                    <span>{{ session('bot_info') }}</span>
                </div>
            @endif

            @if(session('error'))
                <div class="gm-alert error" style="margin: 20px 24px 0 24px;">
